...
subroutine PP_CAT(PP_CAT(WRAP_PRECISION_FAMILY, cfft), WRAP_DIMENSION_FAMILY)(out, inp, num)
#if defined(FFT_1D)
...
end subroutine PP_CAT(PP_CAT(WRAP_PRECISION_FAMILY, cfft), WRAP_DIMENSION_FAMILY)

subroutine PP_CAT(PP_CAT(WRAP_PRECISION_FAMILY, rfft), WRAP_DIMENSION_FAMILY)(out, inp, num)
#if defined(FFT_1D)
    complex(F_COMPLEX_TYPE), allocatable, intent(out) :: out(:)
    real(F_FLOATING_POINT_TYPE),          intent(in)  :: inp(:)
#elif defined(FFT_2D)
    complex(F_COMPLEX_TYPE), allocatable, intent(out) :: out(:, :)
    real(F_FLOATING_POINT_TYPE),          intent(in)  :: inp(:, :)
#elif defined(FFT_3D)
...
#error "Undefined FFT Dimension"
    ...
    fftwplan = PP_CAT(FFTW_PRECISION_FAMILY, plan_dft_r2c_1d)( &
        int(xsize, c_int), &
        inpptr, &
        outptr, &
        fftw_estimate &
    ...
    fftwplan = PP_CAT(FFTW_PRECISION_FAMILY, plan_dft_r2c_2d)( &
        int(ysize, c_int), &
        int(xsize, c_int), &
        inpptr, &
        outptr, &
    ...
    fftwplan = PP_CAT(FFTW_PRECISION_FAMILY, plan_dft_r2c_3d)( &
        int(zsize, c_int), &
        int(ysize, c_int), &
        int(xsize, c_int), &
        inpptr, &
    ...
    fftwplan = PP_CAT(FFTW_PRECISION_FAMILY, plan_dft_r2c)( &
        4_c_int, [ &
            int(tsize, c_int), &
            int(zsize, c_int), &
            int(ysize, c_int), &
...
#error "Undefined FFT Dimension"
    ...
    inpptr = real(0.0, C_FLOATING_POINT_TYPE)
#if defined(FFT_1D)
    inpptr( 1:xinpsize) = real( &
        inp(1:xinpsize), &
        C_FLOATING_POINT_TYPE &
    )
#elif defined(FFT_2D)
...
#error "Undefined FFT Dimension"
    ...
    call PP_CAT(FFTW_PRECISION_FAMILY, execute_dft_r2c)(fftwplan, inpptr, outptr)
...
#if defined(FFT_1D)
    out(1:xhalfsize)          = cmplx(real(outptr), aimag(outptr), F_COMPLEX_TYPE)
...
#error "Undefined FFT Dimension"
    ...
    call PP_CAT(FFTW_PRECISION_FAMILY, destroy_plan)(fftwplan)
    call PP_CAT(FFTW_PRECISION_FAMILY, free)(fftwdata)
end subroutine PP_CAT(PP_CAT(WRAP_PRECISION_FAMILY, rfft), WRAP_DIMENSION_FAMILY)

subroutine PP_CAT(PP_CAT(WRAP_PRECISION_FAMILY, fftshift), WRAP_DIMENSION_FAMILY)(out, inp)
#if defined(FFT_1D)
    complex(F_COMPLEX_TYPE), allocatable, intent(out) :: out(:)
    complex(F_COMPLEX_TYPE),              intent(in)  :: inp(:)
#elif defined(FFT_2D)
...
#error "Undefined FFT Dimension"
#endif
end subroutine PP_CAT(PP_CAT(WRAP_PRECISION_FAMILY, fftshift), WRAP_DIMENSION_FAMILY)
